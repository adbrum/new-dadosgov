...
2. Classify each PR:
   ...
   - **vermelha** → run `gh run view <run-id> --repo <repo> --log-failed`, summarize the real failing assertion, and say whether it looks like a code problem or infrastructure flake. Do not push a fix unless the user asked for autofix.
