...
- `OrganizationTabs` refatorizado com 5 tabs alinhadas à referência data.gouv.fr:
  ...
  3. **Reutilizações** — reuses com cards e métricas.
  4. **Discussões** — cards com título, autor, status aberta/fechada, primeira mensagem e contagem.
  5. **Informações** — estatísticas (datasets, reuses, seguidores, views), membros (avatar, nome, role), informações técnicas (última atualização, ID, data criação, website, NIF).
...
## TICKET-10: Organizations — Search e Filtros (Conexão API)

**Descrição**
Estender `fetchOrganizations()` para suportar pesquisa e filtros.
...
2. **Atualizar `fetchOrganizations()`** em `services/api.ts`:
   - Aceitar: `{ q?, badge?, sort?, page?, pageSize? }`.
3. **Novas funções** em `services/api.ts`:
   - `suggestOrganizations(query, size?)` → `GET /api/1/organizations/suggest/?q=<query>&size=<n>`
   - `fetchOrgBadges()` → `GET /api/1/organizations/badges/`

**Critérios de Aceitação**

- [ ] `fetchOrganizations()` aceita `q` e `badge` e os passa ao backend.
- [ ] `suggestOrganizations()` retorna sugestões.
- [ ] `fetchOrgBadges()` retorna lista de badges.
...
- [ ] Tipos `SpatialZone`, `Granularity`, `GeoLevel` definidos.
