...
**Agravamento — endpoints públicos anónimos (validado em TST, 2026-06-09):** o **mesmo** colapso de IP afeta os endpoints de **pesquisa pública** que populam as páginas de listagem (`GET /api/1/datasets/`, `/api/1/organizations/`, `/api/1/reuses/` com `?q=…`). Ao contrário do `/me`, este tráfego é **anónimo** — não há utilizador para servir de chave ao rate-limit, pelo que **não existe mitigação aplicacional possível**: enquanto o IP real do cliente não chegar ao backend, qualquer limite por IP torna-se um **teto agregado partilhado por todos os visitantes** e, ao fim de algumas centenas de pesquisas por hora **somadas de toda a gente**, a pesquisa deixa de funcionar para todos (negação de serviço trivial — basta um crawler ou um pico de tráfego legítimo). Reproduzido com `scripts/loadtest_search_ratelimit.py` através da emulação do appliance em TST: pedidos com `X-Forwarded-For` **distinto por pedido** (i.e. visitantes diferentes) foram todos colapsados pelo appliance num único balde e passaram a devolver 429. **Subir o teto do rate-limit apenas adia o bloqueio — não o elimina, porque o balde continua partilhado.** Para estes endpoints, **preservar o IP de origem real é a única correção estrutural.**
